...
import { ACTIVE_SESSION_STORAGE_KEY, DIARY_STORAGE_KEY, QUICK_NOTES_STORAGE_KEY, SESSION_HISTORY_STORAGE_KEY, convertQuickNote, editDiary, readDiary, readQuickNotes, readSessions, removeDiary, removeQuickNote, type StoredDiary, type StoredQuickNote, type StoredSession } from '@/lib/local-records';
...
import { activityFilterOptions, buildDiaryViews, buildQuickNoteViews, emptyRecordFilter, filterLearningRecords, isFilterActive, pruneSelection, toSelectableNotes, type LearningRecordFilter } from '@/lib/learning-records-view';
...
export default function HistoryPage() {
  ...
  function toggleSelectAll() { setSelectedIds(allSelected ? new Set<string>() : new Set(visibleViews.map((view) => view.id))); }
  ...
  function handleDeleteDiary(id: string) { persistDiary(removeDiary(diary, id)); }
  function handleDeleteQuickNote(id: string) { persistQuickNotes(removeQuickNote(quickNotes, id)); }
  ...
  return <main className="mx-auto min-h-screen max-w-4xl px-5 py-6 text-stone-900 sm:px-8 sm:py-8"><PrimaryNav active="Learnings" />
    ...
    <div className="mt-8"><LearningsFilters filter={filter} activityOptions={activityFilterOptions(diaryViews)} onFilterChange={setFilter} onClear={() => setFilter(emptyRecordFilter())} /></div>
    {filterActive && <p className="mt-3 text-sm text-stone-600" role="status">Showing {visibleViews.length} of {totalNotes} notes.</p>}
    {visibleNotes.length > 0 && <div className="mt-8"><ExportNotes notes={visibleNotes} selectedIds={selection} onToggleSelectAll={toggleSelectAll} onClearSelection={clearSelection} onExported={handleExported} /></div>}
    ...
    <section className="animate-fade-up stagger-3 mt-12"><div className="mb-4 flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.18em] text-stone-500"><span aria-hidden="true" className="inline-block size-2 rounded-full bg-lavender-300" />Learning diary</h2><span className="text-sm text-stone-500">{filterActive ? `${filteredDiary.length} of ${diaryViews.length} entries` : `${diaryViews.length} entries`}</span></div><div className="rounded-card bg-white p-6 shadow-soft sm:p-8">{filteredDiary.length ? <div className="space-y-6">{filteredDiary.slice().reverse().map((view) => <LearningRecordCard key={view.id} view={view} selected={selection.has(view.id)} onToggleSelect={toggleNote} onEdit={handleSaveEdit} onDelete={handleDeleteDiary} />)}</div> : <p className="px-4 py-14 text-center text-stone-400">{diaryViews.length === 0 ? 'Nothing here yet. Your day is still unwritten.' : 'Nothing matches your search or filters.'}</p>}</div></section>
    <section className="animate-fade-up stagger-4 mt-12"><div className="mb-4 flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.18em] text-stone-500"><span aria-hidden="true" className="inline-block size-2 rounded-full bg-peach-300" />Quick notes</h2><span className="text-sm text-stone-500">{filterActive ? `${filteredQuickNotes.length} of ${quickViews.length} drafts` : `${quickViews.length} drafts`}</span></div><div className="rounded-card bg-white p-6 shadow-soft sm:p-8">{filteredQuickNotes.length ? <div className="space-y-6">{filteredQuickNotes.slice().reverse().map((view) => <LearningRecordCard key={view.id} view={view} selected={selection.has(view.id)} onToggleSelect={toggleNote} onConvert={handleConvert} onDelete={handleDeleteQuickNote} />)}</div> : <p className="px-4 py-14 text-center text-stone-400">{quickViews.length === 0 ? 'Capture a thought on Today — drafts wait here for you.' : 'No quick notes match your search or filters.'}</p>}</div></section>
  </main>;
}
