...
export default function HistoryPage() {
  ...
  return <main className="mx-auto min-h-screen max-w-4xl px-5 py-6 text-stone-900 sm:px-8 sm:py-8"><PrimaryNav active="Learnings" />
    ...
    <section className="animate-fade-up stagger-2 mt-12"><div className="mb-4 flex items-center justify-between"><h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-stone-500">{dateLabel}</h2><span className="text-sm text-stone-500">{sessions.length} completed</span></div><div className="rounded-card bg-white p-2 shadow-soft sm:p-4"><ul className="divide-y divide-stone-100">{activities.map(activity => {
      ...
      return <li key={activity.id} className="grid grid-cols-[2rem_1fr_auto] items-center gap-3 px-4 py-4 sm:px-5"><span className={`flex size-8 items-center justify-center rounded-full text-sm font-semibold ${status.circle}`} aria-label={status.label}>{status.indicator}</span><div className="min-w-0"><h3 className="font-medium text-stone-900">{activity.name}</h3><p className="mt-1 text-sm text-stone-500">{isCompleted && session ? `${new Date(session.startedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} – ${new Date(session.finishedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} · <span className="font-medium text-stone-700">${duration} min</span>` : `${activity.plannedStart} – ${activity.plannedEnd}`}</p></div><span className={`text-right text-sm font-medium ${status.text}`}>{status.label}</span></li>;
    })}</ul></div></section>
