...
export function ExportNotes({ notes, selectedIds, onToggleSelectAll, onClearSelection, onExported }: {
  ...
  return (
    <section className="animate-fade-up mt-10 rounded-2xl bg-white p-6 ring-1 ring-slate-200">
      <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
        <label className="flex min-h-11 cursor-pointer items-center gap-2 text-sm font-medium text-slate-700">
          <input ref={selectAllRef} type="checkbox" checked={allSelected} onChange={onToggleSelectAll} className="size-5 accent-slate-900" />
          Select all notes
        </label>
        <p className="text-sm text-slate-500">{selectedCount} of {total} notes selected</p>
        <div className="ml-auto flex flex-wrap items-center gap-3">
          <label className="flex min-h-11 items-center gap-2 text-sm text-slate-500">
            Format
            <select value={format} onChange={(event) => setFormat(event.target.value === 'md' ? 'md' : 'csv')} className="min-h-11 rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700">
              <option value="csv">CSV (.csv)</option>
              <option value="md">Markdown (.md)</option>
            </select>
          </label>
          <button onClick={exportSelected} disabled={selectedCount === 0} className="min-h-11 rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-700 disabled:opacity-40">
            Export {selectedCount} notes
          </button>
          {selectedCount > 0 && <button onClick={onClearSelection} className="min-h-11 rounded-lg px-3 py-2 text-sm font-medium text-slate-500 transition hover:text-slate-900">Clear</button>}
        </div>
      </div>
    </section>
  );
