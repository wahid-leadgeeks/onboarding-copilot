...
export default function SettingsPage() {
  ...
  return <main className="mx-auto min-h-screen max-w-4xl px-5 py-6 text-slate-900 sm:px-8 sm:py-8"><PrimaryNav active="Settings" />
    <header className="animate-fade-up border-b border-slate-200 pb-8"><p className="text-sm font-medium text-slate-500">Preferences</p><h1 className="text-gradient mt-2 text-4xl font-semibold tracking-tight sm:text-5xl">Settings</h1><p className="mt-3 text-lg text-slate-600">Manage your connection and sync preferences.</p></header>
    <section className="animate-fade-up stagger-1 mt-10 rounded-2xl bg-white p-6 ring-1 ring-slate-200"><div className="flex items-center justify-between"><h2 className="text-lg font-semibold text-slate-900">Connection</h2><button onClick={() => void refreshHealth()} className="min-h-11 rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">Refresh</button></div><p className="mt-3 text-slate-500">{healthError && <span className="text-amber-700">Health check unavailable · </span>}Mode: <span className="font-medium text-slate-700">{health?.mode ?? 'Checking…'}</span></p><div className="mt-4 space-y-2 text-sm"><p><span className={`inline-block w-4 text-center ${health?.integrations.oauth ? 'text-emerald-600' : 'text-slate-400'}`}>{health?.integrations.oauth ? '●' : '○'}</span> Google sign-in</p><p><span className={`inline-block w-4 text-center ${health?.integrations.sheets ? 'text-emerald-600' : 'text-slate-400'}`}>{health?.integrations.sheets ? '●' : '○'}</span> Google Sheets sync</p>{health && !health.integrations.sheets && (health.integrations.sheetsRead || health.integrations.sheetsWrite) && <p className="text-amber-700">⚠️ Sheets is partially configured</p>}<p><span className={`inline-block w-4 text-center ${health?.integrations.ai ? 'text-emerald-600' : 'text-slate-400'}`}>{health?.integrations.ai ? '●' : '○'}</span> AI provider</p></div></section>
    <section className="animate-fade-up stagger-2 mt-6 rounded-2xl bg-white p-6 ring-1 ring-slate-200"><h2 className="text-lg font-semibold text-slate-900">Schedule</h2><p className="mt-3 text-slate-500">Import your onboarding schedule from an Excel or Google Sheets export (.xlsx, .csv, or .tsv). The file is parsed here and kept on this device — nothing is uploaded elsewhere.</p>
      {imported ? <div><p className="mt-4 text-slate-700">Imported schedule · {imported.activities.length} activities · {new Date(imported.importedAt).toLocaleString()}</p><button onClick={removeImported} className="mt-4 min-h-11 rounded-xl border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">Remove imported schedule</button></div>
        : preview ? <div className="animate-pop-in mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="font-medium text-slate-900">{preview.activities.length} activities ready to import{preview.skipped > 0 ? `, ${preview.skipped} skipped` : ''}</p>{preview.skipped > 0 && preview.warnings.length > 0 && <ul className="mt-2 space-y-1 text-xs text-slate-500">{preview.warnings.map((warning, index) => <li key={index}>{warning}</li>)}</ul>}<ul className="mt-3 space-y-1">{preview.activities.slice(0, 5).map((activity, index) => <li key={`${activity.id}-${index}`} className="text-sm text-slate-700">{activity.name} — {activity.plannedStart}–{activity.plannedEnd}</li>)}</ul><div className="mt-4 flex flex-wrap gap-3"><button onClick={confirmImport} className="min-h-11 rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-700">Use this schedule</button><button onClick={discardImport} className="min-h-11 rounded-xl border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">Discard</button></div></div>
        : <label className="mt-4 inline-flex min-h-11 cursor-pointer items-center rounded-xl border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">{parsing ? 'Reading file…' : 'Choose file'}<input type="file" accept=".xlsx,.csv,.tsv" disabled={parsing} onChange={event => void importSchedule(event)} style={{ display: 'none' }} /></label>}
      {importError && <p className="animate-pop-in mt-3 text-sm text-red-700" role="alert">{importError}</p>}
      {importMessage && <p className="mt-3 text-sm" role="status">{importMessage}</p>}
    </section>
    <section className="animate-fade-up stagger-3 mt-6 rounded-2xl bg-white p-6 ring-1 ring-slate-200"><h2 className="text-lg font-semibold text-slate-900">Sync</h2><p className="mt-3 text-slate-500">Completed sessions and learning notes sync after confirmation. Local records remain available if a service is unavailable.</p><button onClick={() => void retryPending()} disabled={!pendingCount} className="mt-4 min-h-11 rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:opacity-40">Retry pending sync</button><p className="mt-3 text-sm" role="status">{pendingCount ? `⏳ ${pendingCount} session${pendingCount === 1 ? '' : 's'} waiting to sync` : '✅ No sessions waiting to sync'}</p></section>
    <section className="animate-fade-up stagger-4 mt-6 rounded-2xl bg-white p-6 ring-1 ring-slate-200"><h2 className="text-lg font-semibold text-slate-900">Guide tour</h2><p className="mt-3 text-slate-500">New to the cockpit? Take a short walkthrough of Today, the timeline, quick notes, and more.</p><a href="/?tour=start" className="mt-4 inline-flex min-h-11 items-center rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">Restart the guide tour</a></section></main>
}
