...
- [x] **App-Wide Non-Technical Guide Tour**: Redesigned onboarding guide tour with warm, human-first copy without technical jargon for non-technical users (HR, marketing, sales, design). Expanded coverage across all 8 main pages (Today, Schedule, Timeline, Reviews, Diary, Feedback, Glossary, Settings) + floating AI Assistant. Moved tour state management into `AppShell` with on-demand relaunch buttons in desktop sidebar and mobile drawer footer, alongside `⌘K` command palette integration.
