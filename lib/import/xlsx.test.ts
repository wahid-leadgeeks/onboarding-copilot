...
import { parseXlsx, parseXlsxSheets } from './xlsx';
...
describe('parseXlsx', () => {
  ...
  it('prefers the Schedule sheet even if it is not the first sheet', () => {
    const multiSheet = zipSync({
      '[Content_Types].xml': strToU8(
        '<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">' +
          '<Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/>' +
          '<Default Extension="xml" ContentType="application/xml"/>' +
      ...
      'xl/worksheets/sheet1.xml': strToU8(
        '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">' +
          '<sheetData><row r="1"><c r="A1" t="inlineStr"><is><t>Guide Info</t></is></c></row></sheetData>' +
          '</worksheet>'
      ),
      'xl/worksheets/sheet2.xml': strToU8(
        '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">' +
          '<sheetData><row r="1"><c r="A1" t="inlineStr"><is><t>Schedule Activity</t></is></c></row></sheetData>' +
          '</worksheet>'
      ),
    });
    expect(parseXlsx(multiSheet)).toEqual([['Schedule Activity']]);
    expect(parseXlsx(multiSheet, 'Guide')).toEqual([['Guide Info']]);

    const sheets = parseXlsxSheets(multiSheet);
    expect(sheets).toHaveLength(2);
    expect(sheets[0].name).toBe('Guide');
    expect(sheets[0].matrix).toEqual([['Guide Info']]);
    expect(sheets[1].name).toBe('Schedule');
    expect(sheets[1].matrix).toEqual([['Schedule Activity']]);
  });
});
